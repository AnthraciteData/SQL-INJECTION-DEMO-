...
//$query = "SELECT * FROM Book WHERE Publisher = '". $_POST['form_input_booktitle'] . "'";

mysqli_multi_query($connect, $query) or die('Error querying database.');
...
<!-- 
		...
		<?php
                    while($row = mysqli_fetch_array($result)){      
                        ...
                        echo "</td><td>".$row['Publisher'];
                        echo "</td><td>".$row['Borrowed_Date'];
			echo "</td><td>".$row['Returned_Date']."</td><tr>";
                    }                                      
                ?>
